...
namespace Api.Data.Core
{
    public static class SecurityHelper
    {
        ...
        public static bool ValidateNonceToken(ClaimsPrincipal principal, ConnectionInfo connection, string tokenKey, IConfigurationSection configuration)
        {
            ...
            var nonce = principal.IsInRole("Service")
        ...
        public static SecurityToken CreateToken(IList<Claim> identityClaims, ConnectionInfo connection, IConfigurationSection section, int expiry)
        {
            ...
            if (expiry > 0)
            {
                expired = DateTime.UtcNow.AddYears(expiry);
        ...
        public static string GenerateServiceToken(IConfigurationSection section, int year)
        {
            var tokeyKey = section[GlobalConstantsProvider.TokenKey];
            ...
            var token = CreateToken(claims, null, section, year);
