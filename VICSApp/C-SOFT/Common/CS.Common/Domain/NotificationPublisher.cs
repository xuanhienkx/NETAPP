...
namespace CS.Common.Domain
{
    public class NotificationPublisher : INotificationPublisher
    {
        ...
        {
            await hubContext.Clients.All.InvokeAsync("Broadcast", message);
            logger.LogDebug($"Broadcast: {message} to all");
        }

        public async Task Unicast(Guid userId, NotificationMessage message)
        {
            await hubContext.Clients.User(userId.ToString()).InvokeAsync("Unicast", message);
        ...
        {
            await hubContext.Clients.Client(clientId).InvokeAsync("Unicast", message);
