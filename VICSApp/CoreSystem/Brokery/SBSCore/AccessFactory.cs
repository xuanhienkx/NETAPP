...
namespace SBSCore
{
   public sealed class AccessFactory : IDisposable
   {
      ...
      {
          var applicationName = ";Application Name=VicsAgency";
#if ( DEBUG)
{  
  applicationName = ";Application Name=VicsAgencyLocal";
}
#endif
         this.connectionString = connectionString + applicationName;
         this.eventLogName = eventLogName;
      }

      public static void CreateFactory()
      {
         string connectionString = Common.Util.SBSConnectionString;
         string eventLogName = ConfigurationManager.AppSettings["EventLog"];
         CreateFactory(connectionString, eventLogName);

         Thread.CurrentThread.CurrentCulture = Thread.CurrentThread.CurrentUICulture = SBSCore.Common.Util.CurrentCulture;
      }
