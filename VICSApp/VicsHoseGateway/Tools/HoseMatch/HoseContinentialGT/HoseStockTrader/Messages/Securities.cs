...
namespace HoseStockTrader.Messages
{
    ...
    internal struct Securities
    {
        ...
        internal string StockSymbol => new string(this.stockSymbol).Trim();

        internal string StockName => new string(this.stockName).Trim();

        internal string IssueDate => new string(this.issueDate).Trim();

        internal string MatureDate => new string(this.matureDate).Trim();

        internal string PriorCloseDate => new string(this.priorCloseDate).Trim();
        //CoveredWarrant
        internal string UnderlyingSymbol => new string(this.underlyingSymbol).Trim();

        internal string IssuerName => new string(this.issuerName).Trim();

        internal string MaturityDate => new string(this.maturityDate).Trim();

        internal string LastTradingDate => new string(this.lastTradingDate).Trim();
        internal string ExerciseRatio => new string(this.exerciseRatio).Trim();
        internal string Type => this.StockType.ToString();
    }
}
