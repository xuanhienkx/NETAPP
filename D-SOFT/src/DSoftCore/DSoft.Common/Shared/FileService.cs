...
public class FileService : IFileService
{
    ...
    {
        var byteBuffer = new byte[ProviderConstants.BufferSize];
        var bytesRead = fromStream.Read(byteBuffer, 0, ProviderConstants.BufferSize);
        try
        {
            while (bytesRead > 0)
            {
                await toStream.WriteAsync(byteBuffer, 0, bytesRead);
                bytesRead = await fromStream.ReadAsync(byteBuffer, 0, ProviderConstants.BufferSize);
            }
    ...
    public async Task<Stream> Read(FileInfo file, bool useCircuitBreaker = false)
    {
        if (!TryGetProvider(file.FullName, out var provider))
            throw new NotImplementedException($"Cannot find file transfer provider for file {file.FullName}");

        var stream = await Execute(() => provider.LoadStream(file), useCircuitBreaker);
        if (stream == null)
            throw new OperationCanceledException($"Cannot read {file.FullName}. Check log for more detail");
