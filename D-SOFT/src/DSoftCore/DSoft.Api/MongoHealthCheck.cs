    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!ping.TryGetValue("ok", out var ok) || (!ok.IsDouble || Math.Abs(ok.AsDouble - 1d) >= double.Epsilon) && (!ok.IsInt32 || ok.AsInt32 != 1))
            {
                ...
                return HealthCheckResult.Unhealthy($"{context.Registration.Name}: {ping.ToJson()}");
            }
            ...
            return _database.Client.Cluster.Description.State == ClusterState.Connected ?
                HealthCheckResult.Healthy($"{context.Registration.Name}: ClusterState.Connected") :
                HealthCheckResult.Unhealthy($"{context.Registration.Name}: ClusterState.Disconnected");
        }
        catch (Exception ex)
        {
            // Exception fired 
            return HealthCheckResult.Unhealthy($"{context.Registration.Name}: Exception {ex.GetType().FullName}", ex);
        }
    }
}
